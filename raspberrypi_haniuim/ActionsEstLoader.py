...
class TSSTG(object):
    ...
    def __init__(self,
        ...
        self.model.load_state_dict(torch.load(weight_file, map_location=torch.device(self.device)))
        self.model.eval()

    def predict(self, pts, image_size):
        if pts.ndim != 3:
            raise ValueError(
                f"pts must have shape (T, 13, 3), got {pts.shape}"
            )

        if pts.shape[1:] != (13, 3):
            raise ValueError(
                f"TSSTG requires (T, 13, 3), got {pts.shape}"
            )
